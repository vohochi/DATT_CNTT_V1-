...
const API_URL =
  'https://cors-anywhere.herokuapp.com/https://api-core.dsp.one/payment/api';
...
): Promise<CreatePaymentHistoryResponse> => {
  try {
...
): Promise<UpdatePaymentHistoryResponse> => {
  try {
...
export const updatePaymentMethod = async (
  ...
  try {
    const response = await axios.patch(
      `${API_URL}/method/updatePaymentMethod/${id}`,
      data,
      {
        headers: {
          allowed_secrets:
...
export const deletePaymentMethod = async (
  ...
  try {
    const response = await axios.delete(
      `${API_URL}/method/deletePaymentMethod/${id}`,
      {
        headers: {
          allowed_secrets:
            'c3f72a381e7f676c21b7fca43fbe60a99aa5ff5dfc76b75993da7bd3032e3f9f',
