...
const TopSaleMain = () => {
  ...
  const handleOpenQuickViewModal = (product) => {
    ...
    setWishlistModalOpen(false); // Đảm bảo wishlist modal đóng lại
  };

  const handleOpenWishlistModal = (product) => {
    setSelectedProduct(product);
    setWishlistModalOpen(true);
    setQuickViewModalOpen(false); // Đảm bảo quick view modal đóng lại
  };
  ...
  return (
    <div className="flex flex-col items-center mb-20">
      ...
      {/* Grid layout cho sản phẩm với responsive */}
      <div className="grid gap-6 grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 w-[78%]">
        {products.map((product) => (
          <div key={product.id} className="flex flex-col items-center w-full relative group">
            <Image
            ...
            <div className="hidden mx-auto lg:flex absolute bottom-[190px] z-20 left-0 right-0 bg-transparent p-2 transform translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
              <div className="flex items-center justify-between mx-auto w-full">
                <div onClick={() => handleOpenQuickViewModal(product)} className="bg-white mx-auto rounded-[50px] p-4 border-2 border-[#FF6565] cursor-pointer">
                  <TbArrowsDiagonal />
                </div>
                <button className="uppercase bg-[#fff] px-4 py-3 rounded-[50px] border-2 border-[#FF6565] transition">
                  Add to cart
                </button>
                <div onClick={() => handleOpenWishlistModal(product)} className="bg-white mx-auto rounded-[50px] p-4 border-2 border-[#FF6565]">
                  <FaRegHeart />
                </div>
              </div>
            </div>
            ...
            <div className="lg:hidden md:hidden flex flex-col items-center mt-4 w-full">
              <div className="flex justify-between w-full">
                <div onClick={() => handleOpenQuickViewModal(product)} className="bg-[#f7f7f7] p-3 border-2 border-[#fff] flex-1 cursor-pointer">
                  <TbArrowsDiagonal className="mx-auto" />
                </div>
                <div onClick={() => handleOpenWishlistModal(product)} className="bg-[#f7f7f7] p-3 border-2 border-[#fff] flex-1">
      ...
      {/* Hiển thị modal khi sản phẩm được chọn */}
      {isQuickViewModalOpen && (
        <QuickViewModal product={selectedProduct} isOpen={isQuickViewModalOpen} onClose={handleCloseModal} />
      )}
      {isWishlistModalOpen && (
        <WishlistModal product={selectedProduct} isOpen={isWishlistModalOpen} onClose={handleCloseModal} />
      )}
    </div>
  );
};
